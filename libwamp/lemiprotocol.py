...
class LemiProtocol(LineReceiver):
    ...
    def processLemiData(self, data):
        ...
        try:
            if self.gpsstate2 == 'P': 
                ## passive mode - no GPS connection -> use ntptime as primary without correction
                evt1 = currenttime
                evt4 = gps_array
            else:
                ## active mode - GPS time is used as primary - please note that 300 milliseconds have been subtracted from GPS time according to the instructions of the LEMI developers
                evt4 = currenttime
                evt1 = gps_array
